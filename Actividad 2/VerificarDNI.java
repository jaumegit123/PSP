	...
	public static boolean ValidarDNI(String dni) {
		...
		if (dni.length() == 9 && Character.isLetter(dni.charAt(8))) {
			do {
				...
				verificarDNI = (charASCII > 47 && charASCII < 58); // verifica si el caràcter està comprés entre 0 i 9
																	// en ASCII
				i++;
			} while (i < dni.length() - 1 && verificarDNI); // si verificarDNI dona fals alguna vegada, el DNI
															// serà incorrecte
		}
		if (verificarDNI) { // en canvi si ix del do while perquè s'ha acabat de recorrer el dni.length(),
							// entrarà dins d'aquest if
			lletra = Character.toUpperCase(dni.charAt(i));
			numeroDNI = Integer.parseInt(dni.substring(0, 8));
			residu = numeroDNI % 23;
			verificarDNI = (lletra == arrLletresDNI[residu]); // si el valor de la operació es el mateix que el ASCII de
																// la lletra, retornarà true
		}
		return verificarDNI;
	}
